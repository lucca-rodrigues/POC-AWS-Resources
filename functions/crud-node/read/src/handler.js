...
/**
 * Lista registros do Postgres usando TypeORM.
 * @param {object} event - Evento recebido do API Gateway com { id } opcional no body.
...
exports.handler = async (event) => {
  ...
  const itens = id
    ? await repository.findBy({ id })
    : await repository.find({ order: { criado_em: 'DESC' } });

  log('crud.item.listado', { quantidade: itens.length, id: id ?? null });
